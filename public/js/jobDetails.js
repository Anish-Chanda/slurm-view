...
function renderPendingReason(data, container) {
  ...
  } else if (data.type === 'QOSMaxCpuPerUserLimit') {
      html += renderQOSMaxGenericPerUserLimitReason(data, 'CPUs');
  } else if (data.type === 'QOSMaxJobsPerUserLimit') {
...
function renderQOSMaxGenericPerUserLimitReason(data, resourceName) {
  const analysis = data.analysis;
  const isNearLimit = parseFloat(analysis.percentUsed) > 95;
  const wouldExceed = analysis.shortfall < 0;
  ...
  const unitDisp = resourceName === 'Jobs' ? '' : ` ${resourceName}`;
  ...
  return `
    <div class="space-y-3">
      <div class="flex items-center text-red-600">
        ...
        <span class="font-semibold">QOS ${resourceName} Per-User Limit Reached</span>
      </div>
      
      <div class="bg-red-50 border border-red-200 rounded-lg p-4 space-y-3">
        <div class="text-sm font-medium text-slate-700 mb-2">QOS: <span class="font-semibold text-slate-900">${analysis.qosName}</span></div>
        <div class="text-sm font-medium text-slate-700 mb-2">User: <span class="font-semibold text-slate-900">${data.job ? data.job.user : 'Unknown'}</span></div>
        ...
        <div class="space-y-2">
          ...
          <div class="flex justify-between items-center">
            <span class="text-slate-600">Your Current Usage:</span>
          ...
          <div class="flex justify-between items-center">
            <span class="text-slate-600">Your Running Jobs:</span>
            <span class="font-semibold text-slate-900">${analysis.runningJobs}</span>
          </div>
        </div>
      </div>
      ...
      ${wouldExceed ? `
        <div class="bg-amber-50 border border-amber-300 rounded-lg p-4">
          <div class="flex items-start">
            ...
            <div class="flex-1">
              <div class="text-sm font-medium text-amber-900">Job Would Exceed Limit</div>
              <p class="text-xs text-amber-700 mt-1">Your job needs ${analysis.shortfallFormatted}${unitDisp} more than you have available in this QOS.</p>
            </div>
          </div>
        </div>
      ` : ''}
...
function renderQOSMaxGenericPerUserLimitReason(data, resourceName) {
  ...
  return `
    <div class="space-y-3">
      ...
      <div class="bg-red-50 border border-red-200 rounded-lg p-4 space-y-3">
        <div class="text-sm font-medium text-slate-700 mb-2">QOS: <span class="font-semibold text-slate-900">${analysis.qosName}</span> | User: <span class="font-semibold text-slate-900">${data.user || 'Unknown'}</span></div>
        
        <div class="space-y-2">
          <div class="flex justify-between items-center">
            <span class="text-slate-600">User Limit:</span>
            <span class="font-semibold text-slate-900">${analysis.limitFormatted}${unitDisp}</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-slate-600">Current User Usage:</span>
            <span class="font-semibold ${isNearLimit ? 'text-red-600' : 'text-slate-900'}">${analysis.currentUsageFormatted}${unitDisp} (${analysis.percentUsed}%)</span>
          </div>
          <div class="flex justify-between items-center">
            <span class="text-slate-600">Available:</span>
