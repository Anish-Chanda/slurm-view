...
const { parseMemoryToMB, formatMemory } = require("../helpers/accountLimits");
    ...
    try {
        ...
        switch (jobData.Reason) {
            ...
            case 'QOSGrpJobsLimit':
                result = analyzeQOSGrpJobsLimit(jobId, jobData);
                break;
            case 'QOSGrpMemLimit':
                result = analyzeQOSGrpMemLimit(jobId, jobData);
                break;
            ...
            case 'QOSMaxMemoryPerUser':
                result = analyzeQOSMaxMemoryPerUser(jobId, jobData);
                break;
            default:
...
/**
 * Analyze a job pending due to QOS jobs limit
 * @param {string} jobId - Job ID
 * @param {Object} jobData - Parsed job data from scontrol
 * @returns {Object} QOSGrpJobsLimit analysis result
 */
const analyzeQOSGrpJobsLimit = (jobId, jobData) => {
    try {
        const qosLimitsData = dataCache.getQOSLimits();
        ...
        if (!qosLimits.grpJobs) {
            return { type: 'Error', message: `QOS '${qosName}' has no jobs limit configured` };
        }
        
        const jobsLimit = parseInt(qosLimits.grpJobs, 10);
        const jobsData = dataCache.getData('jobs');
        let currentUsage = 0;
        let runningJobsCount = 0;
        ...
        console.error(`Error analyzing QOSGrpJobsLimit for job ${jobId}:`, error.message);
        return { type: 'Error', message: error.message };
    }
};

/**
 * Analyze a job pending due to QOS memory limit
 * @param {string} jobId - Job ID
 * @param {Object} jobData - Parsed job data from scontrol
 * @returns {Object} QOSGrpMemLimit analysis result
 */
const analyzeQOSGrpMemLimit = (jobId, jobData) => {
    try {
        const qosLimitsData = dataCache.getQOSLimits();
        if (!qosLimitsData) return { type: 'Error', message: 'QOS limits not available' };
        ...
        let memLimit = null;
        if (qosLimits.grpMem) memLimit = qosLimits.grpMem;
        else if (qosLimits.grpTRES && qosLimits.grpTRES.mem) memLimit = qosLimits.grpTRES.mem;
        
        if (!memLimit) {
            return { type: 'Error', message: `QOS '${qosName}' has no memory limit configured` };
        }
        
        memLimit = parseInt(memLimit, 10);
        
        let jobMemMB = 0;
        if (jobData.MinMemoryNode) {
            jobMemMB = parseInt(jobData.MinMemoryNode.replace(/[^0-9]/g, ''), 10) * (jobData.MinMemoryNode.includes('G') ? 1024 : 1);
        } else if (jobData.ReqTRES && jobData.ReqTRES.mem) {
             if (typeof jobData.ReqTRES.mem === 'string') {
                 const memStr = jobData.ReqTRES.mem;
                 jobMemMB = parseInt(memStr.replace(/[^0-9]/g, ''), 10);
                 if (memStr.includes('G')) jobMemMB *= 1024;
             } else {
                 jobMemMB = jobData.ReqTRES.mem;
             }
        }
        ...
        if (jobsData && jobsData.jobs) {
            jobsData.jobs.forEach(job => {
                ...
                if (jobQOS === qosName && job.job_state === 'RUNNING') {
                    ...
                    if (job.tres_alloc_str && job.tres_alloc_str.includes('mem=')) {
                        const match = job.tres_alloc_str.match(/mem=([0-9]+)([A-Za-z])/);
                        if (match) {
                            const val = parseInt(match[1]);
                            const unit = match[2].toUpperCase();
                            mem = unit === 'G' ? val * 1024 : (unit === 'M' ? val : val);
                        }
                    } else if (job.min_memory) {
                        // Fallback to min_memory from sacct/scontrol
                        if (typeof job.min_memory === 'string' && job.min_memory.includes('G')) {
                            mem = parseInt(job.min_memory) * 1024;
                        } else if (typeof job.min_memory === 'string' && job.min_memory.includes('M')) {
                            mem = parseInt(job.min_memory);
                        } else {
                            mem = parseInt(job.min_memory || 0);
                        }
                    }
                    currentUsage += mem;
                    runningJobsCount++;
        ...
        const available = Math.max(0, memLimit - currentUsage);
        const shortfall = Math.min(0, memLimit - currentUsage - jobMemMB);
        ...
        return {
            ...
            job: {
                requested: {
                    mem: jobMemMB,
                    formatted: formatMem(jobMemMB)
                }
            },
            analysis: {
                qosName: qosName,
                limit: memLimit,
                limitFormatted: formatMem(memLimit),
                currentUsage: currentUsage,
                currentUsageFormatted: formatMem(currentUsage),
                percentUsed: ((currentUsage / memLimit) * 100).toFixed(1),
                available: available,
                availableFormatted: formatMem(available),
                shortfall: shortfall,
                shortfallFormatted: formatMem(Math.abs(shortfall)),
                runningJobs: runningJobsCount
            }
        };
    } catch (error) {
        console.error(`Error analyzing QOSGrpMemLimit for job ${jobId}:`, error.message);
        return { type: 'Error', message: error.message };
    }
};

/**
 * Analyze a job pending due to QOS node limit
 * @param {string} jobId - Job ID
 * @param {Object} jobData - Parsed job data from scontrol
 * @returns {Object} QOSGrpNodeLimit analysis result
 */
const analyzeQOSGrpNodeLimit = (jobId, jobData) => {
    try {
        const qosLimitsData = dataCache.getQOSLimits();
        if (!qosLimitsData) return { type: 'Error', message: 'QOS limits not available' };
        ...
        let nodeLimit = null;
        if (qosLimits.grpNodes) nodeLimit = qosLimits.grpNodes;
        else if (qosLimits.grpTRES && qosLimits.grpTRES.node) nodeLimit = qosLimits.grpTRES.node;
        
        if (!nodeLimit) {
            return { type: 'Error', message: `QOS '${qosName}' has no node limit configured` };
        }
        ...
        console.error(`Error analyzing QOSGrpNodeLimit for job ${jobId}:`, error.message);
        return { type: 'Error', message: error.message };
    }
};
...
function getMemoryMB(value) {
    if (typeof value === 'number') return Number.isFinite(value) ? value : 0;
    return parseMemoryToMB(value);
}
...
function getJobNodeNames(job) {
    if (Array.isArray(job.node_names) && job.node_names.length > 0) {
        return job.node_names.map(nodeName => String(nodeName).trim()).filter(Boolean);
    }

    if (job.node_list && job.node_list !== 'N/A') {
        return expandSlurmHostlist(job.node_list);
    }

    if (typeof job.nodes === 'string' && job.nodes !== 'N/A' && !/^\d+$/.test(job.nodes.trim())) {
        return expandSlurmHostlist(job.nodes);
    }

    return [];
}

function calculateUniqueNodeUsage(jobs, qosName, user) {
    const uniqueNodes = new Set();
    let unknownNodeCount = 0;
    let runningJobsCount = 0;

    jobs.forEach(job => {
        if (!isRunningJobForQOSUser(job, qosName, user)) return;

        runningJobsCount++;
        const nodeNames = getJobNodeNames(job);
        if (nodeNames.length > 0) {
            nodeNames.forEach(nodeName => uniqueNodes.add(nodeName));
            return;
        }

        unknownNodeCount += getJobNodeCount(job);
    });

    return {
        total: uniqueNodes.size + unknownNodeCount,
...
function formatPercent(usage, limit) {
    return ((usage / limit) * 100).toFixed(1);
}

function buildNumericLimitAnalysis(limit, currentUsage, jobRequest, runningJobsCount) {
    const available = Math.max(0, limit - currentUsage);
    const overageAfterJob = Math.max(0, currentUsage + jobRequest - limit);
    const jobShortfall = Math.max(0, jobRequest - available);
    ...
    return {
        ...
        currentUsage: currentUsage,
        currentUsageFormatted: currentUsage.toLocaleString(),
        percentUsed: formatPercent(currentUsage, limit),
        available: available,
        availableFormatted: available.toLocaleString(),
        shortfall: -overageAfterJob,
        shortfallFormatted: overageAfterJob.toLocaleString(),
        jobShortfall: jobShortfall,
        jobShortfallFormatted: jobShortfall.toLocaleString(),
        runningJobs: runningJobsCount
    };
}

function buildMemoryLimitAnalysis(limit, currentUsage, jobRequest, runningJobsCount) {
    const available = Math.max(0, limit - currentUsage);
    const overageAfterJob = Math.max(0, currentUsage + jobRequest - limit);
    const jobShortfall = Math.max(0, jobRequest - available);
    ...
    return {
        limit: limit,
        limitFormatted: formatMemory(limit),
        currentUsage: currentUsage,
        currentUsageFormatted: formatMemory(currentUsage),
        percentUsed: formatPercent(currentUsage, limit),
        available: available,
        availableFormatted: formatMemory(available),
        shortfall: -overageAfterJob,
        shortfallFormatted: formatMemory(overageAfterJob),
        jobShortfall: jobShortfall,
        jobShortfallFormatted: formatMemory(jobShortfall),
        runningJobs: runningJobsCount
    };
}
...
const analyzeQOSMaxCpuPerUserLimit = (jobId, jobData) => {
    try {
        ...
        const analysis = buildNumericLimitAnalysis(cpuLimit, currentUsage, jobCPUs, runningJobsCount);
...
const analyzeQOSMaxMemoryPerUser = (jobId, jobData) => {
    try {
        const qosLimitsData = dataCache.getQOSLimits();
        if (!qosLimitsData) return { type: 'Error', message: 'QOS limits not available' };
        ...
        const memLimit = getMaxTRESPerUserLimit(qosLimits, 'mem');
        if (!memLimit) {
            return { type: 'Error', message: `QOS '${qosName}' has no max memory per user limit configured` };
        }

        const user = getUserNameForAnalysis(jobData);
        if (!user) return { type: 'Error', message: 'Job user not available' };

        const jobMem = getMemoryMB(jobData.ReqTRES?.mem || getJobResourceValue(jobData, 'alloc_memory', 'total_memory'));
        ...
        if (jobsData && jobsData.jobs) {
            jobsData.jobs.forEach(job => {
                ...
                const memToUse = getJobResourceValue(job, 'alloc_memory', 'total_memory');
                currentUsage += getMemoryMB(memToUse);
                runningJobsCount++;
            });
        }

        const analysis = buildMemoryLimitAnalysis(memLimit, currentUsage, jobMem, runningJobsCount);

        return {
            type: 'QOSMaxMemoryPerUser',
            jobId: jobId,
            user: user,
            qosName: qosName,
            job: {
        ...
        console.error(`Error analyzing QOSMaxMemoryPerUser for job ${jobId}:`, error.message);
        return { type: 'Error', message: error.message };
    }
};
...
const analyzeQOSMaxJobsPerUserLimit = (jobId, jobData) => {
    try {
        ...
        const analysis = buildNumericLimitAnalysis(jobsLimit, currentUsage, 1, runningJobsCount);
...
const analyzeQOSMaxNodePerUserLimit = (jobId, jobData) => {
    try {
        ...
        const jobsData = dataCache.getData('jobs');
        let nodeUsage = { total: 0, uniqueNodeCount: 0, unknownNodeCount: 0, runningJobsCount: 0 };
        
        if (jobsData && jobsData.jobs) {
            nodeUsage = calculateUniqueNodeUsage(jobsData.jobs, qosName, user);
        }
        
        const analysis = buildNumericLimitAnalysis(nodeLimit, nodeUsage.total, jobNodes, nodeUsage.runningJobsCount);
        ...
        return {
            ...
            analysis: {
                qosName: qosName,
                ...analysis,
                uniqueNodeCount: nodeUsage.uniqueNodeCount,
                unknownNodeCount: nodeUsage.unknownNodeCount
            }
        };
    } catch (error) {
